    ...
    try {
      ...
      if (path === "/push/subscribe" && request.method === "POST") {
        ...
        if (!sub || !sub.endpoint || !sub.keys || !sub.keys.p256dh || !sub.keys.auth) {
          return json({ ok: false, error: "Abonare invalidă" }, 400);
        }
        const store = (await env.PUSH_KV.get(SUBS_KEY, "json")) || {};
        store[await sha256(sub.endpoint)] = { endpoint: sub.endpoint, keys: sub.keys, added: Date.now() };
...
async function makeVapidJwt(audience, privateJwkJson, publicKeyB64u) {
